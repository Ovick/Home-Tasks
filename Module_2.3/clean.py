    ...
    def loopThroughFiles(dirPath: str):
        ...
        for fileObj in path.iterdir():
            if fileObj.is_file():
                ...
                thread = Thread(target=loopThroughFiles, args=(fileObj,))
                thread.start()
        return
...
def move_file(currentFileLocation: str, newFileLocation: str):
    try:
...
def main():
    ...
    if rootPath.exists():
        ...
        for f in files:
            ...
            thread = Thread(
                target=move_file, args=(currentFileLocation, newFileLocation)
            )
            thread.start()
            thread.join()
